...
`v1.0.1` は永続化の修正を中心としたパッチリリースです。PPT と CAD ワークスペース内の画像は、ユーザーが明示的にワークスペースを消去しない限り、リロード後もより安定して保持されます。

`v1.0.0` は最初の大きな実用リリースです。特に既存の PPT 編集を強化し、NotebookLM から書き出した PPT のテキスト編集と反復修正をより扱いやすくしました。

## v1.0.1 のパッチ更新

- PPT ワークスペースの大きな画像状態を IndexedDB に移し、リロード後に画像が消える問題を修正
- PPT の復元順序を修正し、軽量スナップショットが最新の画像永続化状態を上書きしないよう改善
- CAD のレンダー画像と分析画像を IndexedDB 永続化に移行し、プレースホルダー処理も改善してリロード後の復元安定性を向上
