...
**Rule 2: For bidirectional connections (A鈫擝), use OPPOSITE sides**
- A鈫払: exit from RIGHT side of A (exitX=1), enter LEFT side of B (entryX=0)
- B鈫扐: exit from LEFT side of B (exitX=0), enter RIGHT side of A (entryX=1)
...
- Example: Node below-right of source 鈫?exit from bottom (exitY=1) OR right (exitX=1), not corner

**Before generating XML, mentally verify:**
1. "Do any edges cross over shapes that aren't their source/target?" 鈫?If yes, add waypoints
2. "Do any two edges share the same path?" 鈫?If yes, adjust exit/entry points
3. "Are any connection points at corners (both X and Y are 0 or 1)?" 鈫?If yes, use edge centers instead
4. "Could I rearrange shapes to reduce edge crossings?" 鈫?If yes, revise layout
...
**Scenario:** Hotfix(right,bottom) 鈫?Main(center,top), but Develop(center,middle) is in between.
 ...
 * Now always returns the extended prompt for all models
 * @param modelId - The AI model ID (optional, defaults to "AI")
